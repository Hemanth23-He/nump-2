...
class NTTContext:
    ...
    def precompute_ntt(self, root_of_unity):
        ...
        """
        # Find powers of root of unity using NumPy vectorization
        self.roots_of_unity = np.zeros(self.degree, dtype=object).tolist()
        self.roots_of_unity[0] = 1
        ...
        self.roots_of_unity_inv = np.zeros(self.degree, dtype=object).tolist()
        self.roots_of_unity_inv[0] = 1
        for i in range(1, self.degree):
    ...
    def ftt_fwd(self, coeffs):
        ...
        # We use the FTT input given in the FTT paper - vectorized with NumPy
        coeffs_arr = np.array(coeffs, dtype=object)
        rou_arr = np.array(self.roots_of_unity[:num_coeffs], dtype=object)
        ftt_input = ((coeffs_arr * rou_arr) % self.coeff_modulus).tolist()
    ...
    def ftt_inv(self, coeffs):
        ...
        # We scale down the FTT output given in the FTT paper - vectorized with NumPy
        to_scale_arr = np.array(to_scale_down, dtype=object)
        rou_inv_arr = np.array(self.roots_of_unity_inv[:num_coeffs], dtype=object)
        result = ((to_scale_arr * rou_inv_arr * poly_degree_inv) % self.coeff_modulus).tolist()
...
class FFTContext:
    ...
    def precompute_fft(self):
        ...
        """
        # Vectorized computation of roots of unity using NumPy
        angles = 2 * pi * np.arange(self.fft_length) / self.fft_length
        self.roots_of_unity = (np.cos(angles) + 1j * np.sin(angles)).tolist()
        self.roots_of_unity_inv = (np.cos(-angles) + 1j * np.sin(-angles)).tolist()
